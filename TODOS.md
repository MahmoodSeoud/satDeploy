...
## libsatpush: Replace OpenSSL SHA256 with bundled implementation

**What:** Swap libcrypto dependency in libsatpush for a ~300-line public-domain SHA256
implementation (picohash, libsodium-shaped, or hand-rolled). Keep the
`satpush_sha256_file()` public API stable; only the implementation changes.

**Why:** Drops a heavyweight dep for embedded ARM. Yocto recipe gets ~1.5MB smaller
and the library is more portable for adopters who don't already pull in libcrypto.
Cleaner dependency graph: libsatpush depends only on libcsp + libdtp.

**Context:** satdeploy already links libcrypto for the same purpose, so the libsatpush
extract lands cleanly with OpenSSL still in place. The swap is risk-free but not
urgent. Surfaced in /plan-eng-review (2026-05-21) for the libsatpush extract; deferred
so the extract PR stays focused on the move, not dependency churn.

**Effort:** S (human: ~2 hours / CC: ~15 min)

**Priority:** P3 — improves the embedded story but is not load-bearing for v0.1.

**Depends on:** libsatpush extract PR landing first.

## libsatpush: Add satpush_init_csp_zmq() convenience helper

**What:** Add a one-call helper that initializes libcsp with a ZMQ interface for
adopters who do not already have CSP wired up: `satpush_init_csp_zmq(local_node,
endpoint)` performs csp_init + csp_zmqhub_init + csp_route_start_task in one shot.

**Why:** Lowers first-5-minutes friction for evaluators. New adopters can run the
example CLI without learning libcsp's init API. Important for the "stranger can clone
and use" promise of the v0.1 README.

**Context:** For v0.1, the doxygen note "caller initializes libcsp" is sufficient and
the example CLI in `libsatpush/examples/` demonstrates the pattern. The helper is pure
DX polish that can ship in v0.2 once first external adopter feedback is in. Deferred
from /plan-eng-review (2026-05-21).

**Effort:** S (human: ~1 hour / CC: ~10 min)

**Priority:** P3 — adopter DX improvement; ship after first external adopter reports
init friction.

**Depends on:** libsatpush extract PR; first external adopter feedback ideally.

## libsatpush: Decompose satpush_pull_file() post-extract

**What:** Refactor the 260-line `satpush_pull_file()` (moved from
`satdeploy-agent/src/dtp_client.c:dtp_download_file`) into focused helpers: bitmap
init, sidecar resume detection, retry-round loop, final verify and persist. Keep
the public API unchanged.

**Why:** Currently one giant function inherited from satdeploy-agent. Decomposing
makes it testable per-helper, easier to read for future contributors, and smaller in
code-review surface for subsequent changes. Follows Beck's "make the change easy,
then make the easy change" applied to post-extract sequencing: extract first as a
mechanical move, refactor afterwards in the new clean home.

**Context:** Surfaced in /plan-eng-review (2026-05-21) for the libsatpush extract.
Deferred so the F3.b regression test isolates the extract's correctness from any
refactor side-effects. After the extract PR lands and the baseline is frozen, the
decomposition becomes a clean local change inside libsatpush with the same
regression sweep as the safety net.

**Effort:** M (human: ~3 hours / CC: ~25 min)

**Priority:** P2 — quality improvement; do immediately after the extract lands.

**Depends on:** libsatpush extract PR landing; F3.b baseline frozen as the
regression target.
